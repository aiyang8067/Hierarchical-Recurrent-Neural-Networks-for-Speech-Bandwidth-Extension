...
def __TIMIT_feed_epoch(files,
    ...
    if sort:
        ...
        con_batches=__make_random_batches(con_files[sort_index],batch_size,shuffle)
    else:
        ...
        con_batches=__make_random_batches(con_files,batch_size,shuffle)

    for index,bch_8k in enumerate(batches_8k):

        batch_num=len(bch_8k)
        con=con_batches[index]
        ...
        if len(mask_all0_index!=0):
            ...
            con=con[:,:-(len(mask_all0_index)/con_frame_size*con_dim)]
        ...
        con=np.pad(con,[[0,0],[0,batch_seq_len/con_frame_size*con_dim-con.shape[1]]],'constant')
        ...
        for i in xrange(batch_seq_len // seq_len):
            ...
            subcon=con[:,i*seq_len/con_frame_size*con_dim:(i+1)*seq_len/con_frame_size*con_dim]
            yield (subbatch_8k, subbatch_up,reset, end_flag,submask,subcon,batch_num,subbatch_8k_real)
...
def TIMIT_train_feed_epoch(*args):
    ...
    data_con_path=find_dataset(__train_con(__TIMIT_file))
    files=[]
    files.append(np.load(data_path_8k))
    files.append(np.load(data_path_up))
    mask_files=np.load(data_mask_path)
    con_files=np.load(data_con_path)
    shuffle=True
    sort=True
    generator = __TIMIT_feed_epoch(files, mask_files,con_files,shuffle,sort,*args)
    return generator
...
def TIMIT_valid_feed_epoch(*args):
    ...
    data_con_path=find_dataset(__valid_con(__TIMIT_file))
    files=[]
    files.append(np.load(data_path_8k))
    files.append(np.load(data_path_up))
    mask_files=np.load(data_mask_path)
...
def TIMIT_test_feed_epoch(*args):
    ...
    data_con_path=find_dataset(__test_con(__TIMIT_file))
    files=[]
    files.append(np.load(data_path_8k))
    files.append(np.load(data_path_up))
    mask_files=np.load(data_mask_path)
