...
BIG_FRAME_SIZE = args.big_frame_size # how many samples per big frame (8)
FRAME_SIZE = args.frame_size # How many samples per frame (2)
...
train_fn = theano.function(
    [sequences_8k,sequences_up, big_h0, h0, reset, mask,batch_size,lr],
...
valid_fn = theano.function(
    [sequences_8k,sequences_up, big_h0,h0, reset, mask,batch_size],
    [cost_sum, accuracy,mask_sum,new_big_h0,new_h0],
    on_unused_input='warn'
)

test_fn=theano.function(
    [sequences_8k,sequences_up, big_h0,h0, reset, mask,batch_size],
    [cost_sum,accuracy,mask_sum,lib.ops.softmax_and_no_sample(sample_level_outputs.reshape((batch_size,SEQ_LEN,Q_LEVELS))),new_big_h0,new_h0],
    on_unused_input='warn'
)
...
def generate_and_save_samples(tag):
    ...
    for seqs_g_8k,seqs_g_up, reset_g, end_flag_g,mask_g,batch_g,seqs_g_8k_real in data_feeder:
        if reset_g==1:
            ...
            count+=1
        cost_g, accuracy_g,mask_sum_g,sample, big_h0_g,h0_g = test_fn(seqs_g_8k,seqs_g_up, big_h0_g,h0_g, reset_g, mask_g,batch_g)
...
def monitor(data_feeder):
    ...
    for _seqs_8k,_seqs_up, _reset, _end_flag,_mask,_batch,_seqs_8k_real in _data_feeder:
        if _reset==1:
        ...
        _cost, _accuracy,_mask_sum,_big_h0,_h0 = valid_fn(_seqs_8k,_seqs_up, _big_h0,_h0, _reset, _mask,_batch)
...
while True:
    ...
    seqs_8k, seqs_up,reset, end_flag,mask,batch_num,seqs_8k_real = mini_batch
    if reset==1:
        big_h0=numpy.zeros((batch_num, N_BIG_RNN, H0_MULT*DIM), dtype='float32')
        h0 = numpy.zeros((batch_num, N_RNN, H0_MULT*DIM), dtype='float32')

    start_time = time()
    cost,big_h0,h0 = train_fn(seqs_8k, seqs_up, big_h0,h0, reset, mask,batch_num,learning_rate)
